...
Five new analytics surfaces, all read-only, all derived from existing data (trades table + engine-push blobs). No schema changes, no changes to `/api/trades`, `/api/health`, or `/api/engine/dashboard` shapes.

- **Portfolio Risk & Greeks panel** (`GET /api/analytics/risk`) — net entry delta, estimated theta/day (linear decay of remaining premium), premium remaining at last engine mark, unrealized P/L with ITM flag, collateral at risk, CSP max loss, next-expiry pressure.
- **Collateral Release Schedule** (`GET /api/analytics/exposure`) — expiry ladder: capital parked per expiration, premium left, DTE, plus engine funding-queue chips. TradesViz-style capital-at-risk.
- **Payoff-at-expiry diagrams** — chart icon on each CSP/CC row in Open Positions opens an OptionStrat-style P/L curve held to expiry, split at breakeven, with BE + estimated-underlying markers. CC rows with a recorded share lot combine shares+call (wheel payoff). Pure math in `src/utils/payoff.js` (headlessly unit-tested).
- **P/L Attribution by Underlying** (`GET /api/analytics/attribution`) — realized P/L per symbol (same formula/units as `stats.js`: short legs entry-close, commission-adjusted), W/L + win rate, capital-weighted ROI, open collateral and unrealized.
- **Wheel Cycle Tracker** (`GET /api/analytics/cycles`) — trades grouped into cycles per ticker, roll chains fold into their root cycle; realized premium (terminal legs only), days held, ROI and annualized APY.

### E2E test kit
- `scripts/e2e/seed.mjs` — realistic wheel-book seeder for scratch instances (pass `PORT`, refuses 8096; wipes demo trades; open CSP/CC, roll chain, closed winners/losses, engine dashboard push, XOM lot).

### Verification
- Every endpoint asserted against hand-computed values on a scratch instance (port 8296, isolated DATA_DIR, NODE_ENV=production).
- Full playwright render sweep: 32 analytics-page assertions + 10 payoff-modal assertions, twice green (screenshots archived in build evidence).
